...
let loc_of lex = Naming.make lex.Ulexbuf.pos_start lex.Ulexbuf.pos_end
...
and token_aux ({ Ulexbuf.stream;_ } as lexbuf) =
  ...
  | '_'                      -> f (); Printf.printf("_"); Parser.UNDERSCORE
  | '('                      -> f (); Parser.LPAREN
  | ')'                      -> f (); Parser.RPAREN
  | ','                      -> f (); Parser.COMMA
  | ':'                      -> f (); Parser.COLON
  ...
  | prefixop                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.PREFIXOP op
  | infixop0                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.INFIXOP0 op
  | infixop1                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.INFIXOP1 op
  | infixop2                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.INFIXOP2 op
  (* Comes before infixop3 because ** matches the infixop3 pattern too *)
  | infixop4                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.INFIXOP4 op
  | infixop3                 -> f (); let op = Naming.locate ~loc:(loc_of lexbuf) (Naming.Name(Ulexbuf.lexeme lexbuf)) in
                                      Parser.INFIXOP3 op
...
let read_file parse fn =
  ...
  (* Any errors when opening or closing a file are fatal. *)
    Sys_error msg -> raise (Ulexbuf.error ~loc:Naming.Nowhere (Ulexbuf.SysError msg))
